...
async function admin(req, res) {
  const auth = requireAuth(req);
  const staffRoles = ['owner', 'per_admin', 'admin', 'admin_assistant', 'operator'];
  const fundRoles = ['owner', 'per_admin', 'admin', 'admin_assistant'];
  if (!auth || !staffRoles.includes(auth.role)) return jsonResponse(res, 403, { error: 'Admin only' });

  try {
    const db = getDb();
    const { data: house } = await db.from('house_stats').select('*').eq('id', 1).single();
    const { data: users } = await db.from('users').select('id, username, balance, wins, rounds, referred_by, is_admin, role').order('created_at', { ascending: false });
    ...
    if (req.method === 'POST') {
      if (!fundRoles.includes(auth.role)) return jsonResponse(res, 403, { error: 'No permission to add funds' });
