...
async function bet(req, res) {
  ...
  try {
    ...
    if (user.last_trade_at) {
      const elapsed = (Date.now() - new Date(user.last_trade_at).getTime()) / 1000;
      if (elapsed < TRADE_COOLDOWN_SEC) {
        const retryAfter = Math.ceil(TRADE_COOLDOWN_SEC - elapsed);
        return jsonResponse(res, 429, {
          error: `Please wait ${retryAfter} second${retryAfter === 1 ? '' : 's'} — previous trade still processing`,
          retryAfter
        });
      }
    }
    ...
    const { data: existing } = await db.from('bets').select('id, amount')
      .eq('user_id', auth.id).eq('round_id', info.roundId).eq('number', num).maybeSingle();

    const { data: allBets } = await db.from('bets').select('number')
      .eq('user_id', auth.id).eq('round_id', info.roundId);
    const distinctNumbers = new Set((allBets || []).map(b => b.number));
    if (!existing && distinctNumbers.size >= 6) {
      return jsonResponse(res, 400, { error: 'Maximum 6 trades per round (one per number)' });
    }

    const isFirstBetThisRound = (allBets || []).length === 0;
    ...
    if (existing) {
      await db.from('bets').update({ amount: Number(existing.amount) + amt }).eq('id', existing.id);
    } else {
    ...
    jsonResponse(res, 200, {
      ...
      balance: newBal,
      cooldownSec: TRADE_COOLDOWN_SEC
    });
  } catch (e) {
    jsonResponse(res, 500, { error: e.message });
  }
...
async function leaderboard(req, res) {
  ...
  try {
    ...
    const masked = await maskLeaderboard(users || [], db, viewer);
...
async function admin(req, res) {
  ...
  try {
    ...
    if (perms.can_manage_users) payload.users = maskUserRows(users, auth);
  ...
  if (req.method === 'GET') {
    ...
    try {
      const { data: users, error } = await db.from('users')
        ...
        .order('created_at', { ascending: false });
      if (error) return jsonResponse(res, 500, { error: error.message });
      return jsonResponse(res, 200, { users: maskUserRows(users || [], auth) });
    } catch (e) {
  ...
  try {
    ...
    if (action === 'pending' && req.method === 'GET') {
      ...
      const { data, error } = await db.from('deposit_requests')
        ...
        .limit(50);
      if (error) return jsonResponse(res, 500, { error: error.message });
      return jsonResponse(res, 200, { requests: await maskRequestRows(data || [], db, auth) });
    }
    ...
    if (action === 'approve' && req.method === 'POST') {
      ...
      return jsonResponse(res, 200, {
        newBalance: newBal,
        username: maskUserRow(u, auth).username,
...
async function withdrawalsRoute(req, res) {
  ...
  try {
    ...
    if (action === 'sent' && req.method === 'POST') {
      ...
      if (error) return jsonResponse(res, 500, { error: error.message });
      return jsonResponse(res, 200, {
        username: maskUserRow(wu || { username: reqRow.username, role: 'player' }, auth).username,
        amount: reqRow.amount,
        sizeKB: proofSizeKb
      });
    }
    ...
    if (action === 'reject' && req.method === 'POST') {
      ...
      return jsonResponse(res, 200, {
        ...
        username: maskUserRow(u || { username: reqRow.username, role: 'player' }, auth).username
      });
    }
...
async function adminBan(req, res) {
  ...
  try {
    ...
    return jsonResponse(res, 200, {
      ok: true,
      username: maskUserRow(target, auth).username,
      banned: shouldBan
    });
  } catch (e) {
    return jsonResponse(res, 500, { error: e.message });
