...
const { isPromoUser, walletBalance, walletField } = require('./promo-wallet');
const {
  filterOwnerRows,
  visibleUserCount,
  getHiddenOwnerCount,
...
async function login(req, res) {
  ...
  try {
    ...
    const db = getDb();
    const { data: user, error: userErr } = await db.from('users').select('*').ilike('username', username.trim()).maybeSingle();
    if (userErr) {
      return jsonResponse(res, 500, {
        error: /row-level security|permission denied/i.test(userErr.message || '')
          ? 'Database access blocked — check SUPABASE_SERVICE_ROLE_KEY on Vercel (must be service_role, not anon).'
          : userErr.message
      });
    }
    if (!user) {
    ...
    if (!user.password_hash) {
      return jsonResponse(res, 403, { error: 'Password not set — ask admin to reset your password' });
    }
...
async function changePassword(req, res) {
  ...
  try {
    ...
    if (!currentPassword || !newPassword) {
      return jsonResponse(res, 400, { error: 'Current and new password required' });
    }
    if (newPassword.length < 4) {
      return jsonResponse(res, 400, { error: 'New password must be at least 4 characters' });
    }
    if (currentPassword === newPassword) {
      return jsonResponse(res, 400, { error: 'New password must be different' });
    }

    const db = getDb();
    const { data: user, error } = await db.from('users').select('id, password_hash, is_banned').eq('id', auth.id).maybeSingle();
    if (error || !user) return jsonResponse(res, 401, { error: 'Session expired — please sign in again' });
    if (user.is_banned) return jsonResponse(res, 403, { error: 'Account suspended' });
    if (!user.password_hash || !(await checkPassword(currentPassword, user.password_hash))) {
      return jsonResponse(res, 401, { error: 'Current password is incorrect' });
    }

    const passwordHash = await bcrypt.hash(newPassword, 10);
    const { error: updErr } = await db.from('users').update({ password_hash: passwordHash }).eq('id', auth.id);
    if (updErr) return jsonResponse(res, 500, { error: updErr.message });

    await writeAudit(db, 'password_changed', auth.username, auth.username, 'User changed own password');
...
async function adminResetPassword(req, res) {
  ...
  try {
    ...
    if (!perms.can_manage_users) return jsonResponse(res, 403, { error: 'No permission to reset passwords' });

    const { userId, newPassword } = parseBody(req);
    if (!userId || !newPassword) return jsonResponse(res, 400, { error: 'userId and newPassword required' });
    if (newPassword.length < 4) return jsonResponse(res, 400, { error: 'Password must be at least 4 characters' });

    const { data: target, error: tErr } = await db.from('users').select('id, username, role').eq('id', userId).maybeSingle();
    if (tErr || !target) return jsonResponse(res, 404, { error: 'User not found' });
    if (target.id === auth.id) {
      return jsonResponse(res, 400, { error: 'Use Profile → Change password for your own account' });
    }
    const targetRole = target.role || 'player';
    if ((ROLE_RANK[auth.role] || 0) <= (ROLE_RANK[targetRole] || 0) && targetRole !== 'player') {
      return jsonResponse(res, 403, { error: 'Cannot reset password for this user' });
    }

    const passwordHash = await bcrypt.hash(newPassword, 10);
    const { error: updErr } = await db.from('users').update({ password_hash: passwordHash }).eq('id', userId);
    if (updErr) return jsonResponse(res, 500, { error: updErr.message });

    await writeAudit(db, 'password_reset', auth.username, target.username, 'Admin reset password');
    jsonResponse(res, 200, { ok: true, username: target.username });
  } catch (e) {
    jsonResponse(res, 500, { error: e.message });
  }
}
...
async function bet(req, res) {
  ...
  try {
    ...
    if (userUpdErr && /last_trade_at/i.test(userUpdErr.message || '')) {
      const fallbackPatch = { rounds: userPatch.rounds };
      if (isPromo) fallbackPatch.promo_balance = newBal;
      else fallbackPatch.balance = newBal;
      await db.from('users').update(fallbackPatch).eq('id', auth.id);
    } else if (userUpdErr) {
    ...
    for (const num of numbers) {
      ...
      if (existing) {
        ...
        if (isPromo) betRow.is_promo = true;
        let ins = await db.from('bets').insert(betRow);
        if (ins.error && /is_promo/i.test(ins.error.message || '')) {
          delete betRow.is_promo;
          ins = await db.from('bets').insert(betRow);
        }
    ...
    if (isPromo) roundPatch.promo_pool = Number(current.promo_pool || 0) + totalCost;
    else roundPatch.real_pool = Number(current.real_pool || 0) + totalCost;

    let roundUpd = await db.from('rounds').update(roundPatch).eq('id', info.roundId);
    if (roundUpd.error && /real_pool|promo_pool/i.test(roundUpd.error.message || '')) {
      await db.from('rounds').update({
        bets: roundPatch.bets,
        pool: roundPatch.pool,
        player_count: roundPatch.player_count
      }).eq('id', info.roundId);
    } else if (roundUpd.error) {
      throw new Error(roundUpd.error.message);
    }
...
async function admin(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const col = walletField(user);
      const cur = walletBalance(user);
      const next = cur + amt;
      const patch = col === 'promo_balance' ? { promo_balance: next } : { balance: next };
      await db.from('users').update(patch).eq('id', user.id);
      return jsonResponse(res, 200, { success: true, newBalance: next, promoWallet: col === 'promo_balance' });
    }
    ...
    if (perms.can_manage_users) {
      const { data } = await db.from('users')
        .select('id, username, balance, promo_balance, wins, rounds, referred_by, is_admin, role, is_banned')
        .order('created_at', { ascending: false });
      users = data || [];
    ...
    if (perms.can_view_financials) {
      ...
      payload.promo = {
        pool: Number(house?.promo_pool || 0),
        payouts: Number(house?.promo_payouts || 0),
        net: Number(house?.promo_net || 0)
      };
    ...
    if (perms.can_manage_rounds || perms.can_view_financials) {
      payload.rounds = (rounds || []).map(r => ({
        id: r.id,
        winner: r.winner,
        pool: Number(r.real_pool ?? r.pool),
        fullPool: Number(r.pool),
        promoPool: Number(r.promo_pool || 0),
        housePL: Number(r.house_pl),
        promoPL: Number(r.promo_pl || 0),
...
async function adminRoles(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      const { data: currentUser } = await db.from('users').select('role, balance, promo_balance').eq('id', userId).maybeSingle();
      const patch = {
        role,
      ...
      if (role === 'control_player') {
        ...
        if (currentUser?.role !== 'control_player') {
          ...
          patch.promo_balance = Number(currentUser?.promo_balance || 0) + Number(currentUser?.balance || 0);
          patch.balance = 0;
        }
      } else if (currentUser?.role === 'control_player') {
        patch.balance = Number(currentUser?.balance || 0) + Number(currentUser?.promo_balance || 0);
        patch.promo_balance = 0;
      }

      let { error } = await db.from('users').update(patch).eq('id', userId);
      if (error && /promo_balance|control_win_rate/i.test(error.message || '')) {
        const slim = { role, is_admin: STAFF_ROLES.includes(role) };
        if (role === 'control_player' && /control_win_rate/i.test(error.message || '')) {
          return jsonResponse(res, 400, {
            error: 'Run v13 + v14 SQL migrations in Supabase (control player + promo wallet), then try again.'
          });
        }
        ({ error } = await db.from('users').update(slim).eq('id', userId));
      }
      if (error && role === 'control_player' && /control_player|users_role_check/i.test(error.message || '')) {
...
async function depositsRoute(req, res) {
  ...
  try {
    ...
    if (action === 'approve' && req.method === 'POST') {
      ...
      const newBal = walletBalance(u) + Number(reqRow.amount);
      const patch = isPromoUser(u) ? { promo_balance: newBal } : { balance: newBal };
...
async function withdrawalsRoute(req, res) {
  ...
  try {
    if (action === 'submit' && req.method === 'POST') {
      ...
      if (uErr || !u) return jsonResponse(res, 404, { error: 'User not found' });
      if (u.is_banned) return jsonResponse(res, 403, { error: 'Account suspended — contact support' });
      const curBal = walletBalance(u);
      if (curBal < amt) return jsonResponse(res, 400, { error: 'Insufficient balance' });

      const newBal = curBal - amt;
      const patch = isPromoUser(u) ? { promo_balance: newBal } : { balance: newBal };
      await db.from('users').update(patch).eq('id', auth.id);
      ...
      if (error) {
        const rollback = isPromoUser(u) ? { promo_balance: curBal } : { balance: curBal };
        await db.from('users').update(rollback).eq('id', auth.id);
        return jsonResponse(res, 500, { error: error.message });
      }
    ...
    if (action === 'reject' && req.method === 'POST') {
      ...
      if (u) {
        const refund = walletBalance(u) + Number(reqRow.amount);
        const patch = isPromoUser(u) ? { promo_balance: refund } : { balance: refund };
        await db.from('users').update(patch).eq('id', u.id);
      }
      const { error } = await db.from('withdrawal_requests').update({
        status: 'rejected',
