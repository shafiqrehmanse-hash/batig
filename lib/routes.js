...
async function bet(req, res) {
  ...
  try {
    ...
    const existingAmounts = [...new Set((allBets || []).map((b) => Number(b.amount)))];

    if (existingAmounts.length === 1 && existingAmounts[0] !== amt) {
      return jsonResponse(res, 400, {
        error: `This round uses PKR ${existingAmounts[0].toLocaleString()} per number — same stake on all picks`
      });
    }

    const toPlace = numbers.filter((n) => !existingByNum.has(n));
    if (!toPlace.length) {
      return jsonResponse(res, 400, { error: 'Selected numbers already have trades this round' });
    }

    const distinctAfter = existingByNum.size + toPlace.length;
    if (distinctAfter > 6) {
      return jsonResponse(res, 400, { error: 'Maximum 6 numbers per round' });
    }

    const totalCost = amt * toPlace.length;
    if (Number(user.balance) < totalCost) {
      return jsonResponse(res, 400, { error: 'Insufficient balance for ' + toPlace.length + ' trade(s)' });
    }
    ...
    const current = await ensureRound(info.roundId);
    const bets = [...(current.bets || [0, 0, 0, 0, 0, 0])].map(Number);
    toPlace.forEach((num) => { bets[num - 1] += amt; });
    ...
    jsonResponse(res, 200, {
      success: true,
      placed: toPlace.map((n) => ({ number: n, amount: amt })),
