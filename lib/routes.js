...
const { getRoundInfo, jsonResponse, parseBody } = require('./game');
const { ensureRound, resolveRound } = require('./resolve');
...
async function round(req, res) {
  ...
  try {
    ...
    if (!current.resolved && info.phase === 'rolling' && info.sec >= 50) {
