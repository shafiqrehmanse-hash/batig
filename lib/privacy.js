...
async function filterLeaderboard(entries, db, viewerAuth = null) {
  if (isViewerOwner(viewerAuth)) {
    return (entries || []).filter((e) => (e.role || 'player') !== 'control_player');
  }
  const ownerNames = await getOwnerUsernames(db);
  return (entries || []).filter((e) => {
    if ((e.role || 'player') === 'control_player') return false;
    return !ownerNames.has((e.username || '').toLowerCase());
  });
}
